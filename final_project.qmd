...
# Score
# 5a. Reorder countries by descending happiness score
# 6a. Specify data, x axis, y axis, color, and line type
# 6b. Set the line width
# 6c. Specify the color of each line and the corresponding label on the
# legend, using a color blind friendly palette
# 6d. Specify the type of each line and the corresponding label on the
# legend, dual encoding for discriminatory/color blindness
# 6e. Add the labels for the title, each axis, and the color/line type
# 6f. Add black and white theme
```
...
```{r}
# 5a. Reorder countries by descending happiness score
countries_data$Country <- factor(countries_data$Country, levels = c("Switzerland", "Iceland", "Denmark", "Norway", "Canada", "Rwanda", "Benin", "Syria", "Burundi", "Togo")) #reorder countries by
...
ggplot(
  ...
  scale_colour_manual(
    ...
    labels = c(
      ...
      "Togo - Lowest Happiness Score" #manually label each country with corresponding happiness
      # score rank
    )
  ) +
  ...
  ylim(0, 10) + #ensure y axis from zero to ten, range of happiness rank
  labs(
